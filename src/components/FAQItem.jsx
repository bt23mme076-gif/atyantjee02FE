...
export default function FAQItem({ question, answer, open, onToggle, dark = false }) {
  return (
    <div className={`rounded-[1.5rem] border transition-colors ${
      dark 
      ...
      <button type="button" onClick={onToggle} className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left">
        <span className={`text-base font-semibold ${dark ? 'text-white' : 'text-[#0B0F2E]'}`}>{question}</span>
      ...
      <AnimatePresence initial={false}>
        {open ? (
          ...
          >
            <div className={`px-6 pb-5 text-sm leading-7 ${dark ? 'text-slate-300' : 'text-slate-600'}`}>{answer}</div>
          </motion.div>
        ) : null}
      </AnimatePresence>
    </div>
