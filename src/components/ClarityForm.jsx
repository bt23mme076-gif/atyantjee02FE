...
const inputCls =
  'w-full rounded-lg border border-slate-300 p-3 outline-none focus:border-[#FF6B2B] focus:ring-1 focus:ring-[#FF6B2B]';
    ...
    <div className="bg-white rounded-2xl shadow-[0_20px_60px_rgba(15,23,42,0.06)] border border-slate-200 p-6 sm:p-8 max-w-2xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-600 mt-2 text-sm">
          Tell us about your situation so we can give you the best path forward.
        </p>
        <div className="flex gap-2 mt-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className={`h-2 flex-1 rounded-full ${step >= i ? 'bg-[#FF6B2B]' : 'bg-slate-200'}`}
            />
          ))}
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={inputCls}
                  placeholder="JEE Main, MHT-CET, etc."
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={inputCls}
                  placeholder="e.g. 1.2 Lakh or 92%"
                />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="mt-8 flex items-center justify-between">
          {step > 1 ? (
            <button
              type="button"
              onClick={handleBack}
              className="text-sm font-semibold text-slate-600 hover:text-[#0B0F2E]"
            >
              Back
            </button>
          ) : (
          ...
          <button
            type="submit"
            className="rounded-full bg-[#FF6B2B] px-8 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-[#ff7a42] hover:scale-105"
          >
