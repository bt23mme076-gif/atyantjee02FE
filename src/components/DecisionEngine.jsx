...
const STEPS = [
  {
    ...
    icon: User,
    q: 'Who are you?',
    sub: 'Tell us a bit about yourself and where to send your results.',
  },
  {
    key: 'academic',
    label: 'Academic',
    icon: GraduationCap,
    q: 'Your academic profile?',
    sub: 'This helps us match you to the right college tier.',
  },
  {
    key: 'confusion',
    label: 'Confusion',
    icon: Flame,
    q: "What's your biggest confusion?",
    sub: 'Be honest. This is where most students get stuck.',
  },
];
...
const TILE_COLORS = [
  {
    base: 'border-slate-200 bg-white hover:border-blue-300 hover:shadow-md',
    active: 'border-blue-500 bg-blue-50 shadow-[0_0_16px_rgba(59,130,246,0.15)]',
  ...
  {
    base: 'border-slate-200 bg-white hover:border-purple-300 hover:shadow-md',
    active: 'border-purple-500 bg-purple-50 shadow-[0_0_16px_rgba(139,92,246,0.15)]',
    iconBase: 'bg-purple-100 text-purple-600',
    iconActive: 'bg-purple-500 text-white shadow-md',
    check: 'text-purple-500',
    lbl: 'text-purple-900',
  },
  {
    base: 'border-slate-200 bg-white hover:border-orange-300 hover:shadow-md',
    active: 'border-orange-500 bg-orange-50 shadow-[0_0_16px_rgba(249,115,22,0.15)]',
    iconBase: 'bg-orange-100 text-orange-600',
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <div
          className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl transition-all ${selected ? c.iconActive : c.iconBase}`}
        >
...
export default function DecisionEngine() {
  ...
  const validateStep = () => {
    if (
      activeStep === 1 &&
      (!fullName.trim() || !whatsapp.trim() || !gender || !preferredLanguage.trim() || !homeState)
    )
      return false;
      ...
      <div className="relative z-10 mx-auto max-w-5xl">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 rounded-full bg-white/10 border border-white/15 px-5 py-2 mb-5">
            ...
            <span className="text-xs font-black uppercase tracking-widest text-[#FFB38E]">
          ...
          <p className="mt-3 text-white/50 text-base">
            3 quick sections. Your exact college roadmap.
          </p>
        </div>
                ...
                <div className="mb-8 pb-6 border-b border-white/10">
                  ...
                  <AnimatePresence mode="wait">
                    {fullName ? (
                      ...
                      <motion.p key="ph" className="text-sm text-white/50 font-bold">
                        Your roadmap awaits...
                      </motion.p>
                    )}
                  </AnimatePresence>
                    ...
                    return (
                      <div key={s.key} className="flex gap-3">
                        <div className="flex flex-col items-center">
                          <div
                            className={`h-7 w-7 rounded-full flex items-center justify-center shrink-0 transition-all duration-400 ${
                              isDone
                          ...
                          >
                            {isDone ? (
                              ...
                              <Icon
                                className={`h-3.5 w-3.5 ${isCurrent ? 'text-white' : 'text-white/20'}`}
                              />
                            )}
                          </div>
                          {idx < STEPS.length - 1 && (
                        ...
                        <div className="pb-2 pt-1 flex-1 min-w-0">
                          <p
                            className={`text-xs font-black mb-1 ${isCurrent ? 'text-white' : isDone ? 'text-white/80' : 'text-white/40'}`}
                            ...
                            >
                              {s.key === 'basic' && (
                                <>
                                  ...
                                  <p className="text-[11px] text-white/60 truncate">{homeState}</p>
                                </>
                              )}
                              {s.key === 'academic' && (
                                <>
                                  <p className="text-[11px] text-white/75">
                                    {examType} · CRL {rankInput}
                                  </p>
                                  <p className="text-[11px] text-white/60">Cat: {category}</p>
                                </>
                              )}
                              {s.key === 'confusion' && (
                                <p className="text-[11px] text-white/75 truncate">{confusion}</p>
                              )}
                            </motion.div>
                          )}
                          {isCurrent && (
            ...
            <div className="bg-white flex flex-col">
              <div className="p-7 sm:p-10 flex-1">
                <div className="flex items-center justify-between mb-5">
                  <span
                    className="rounded-full px-3.5 py-1.5 text-xs font-black text-white shadow-sm"
                    style={gradStyle}
                  >
                    {step.label}
                  </span>
                  <span className="text-xs font-bold text-slate-300 tabular-nums">
                    {activeStep}&thinsp;/&thinsp;{STEPS.length}
                  </span>
                </div>
                ...
                <div className="min-h-[340px]">
                  <AnimatePresence mode="wait" custom={direction}>
                    ...
                    >
                      <div>
                        <h3 className="text-2xl sm:text-3xl font-black text-slate-800 leading-tight mb-2">
                          {step.q}
                        </h3>
                      ...
                      {activeStep === 3 && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {[
                            {
                              id: 'College vs Branch',
                              icon: GitBranch,
                              desc: 'Should I compromise branch for a better college?',
                            },
                            {
                              id: 'Branch vs Career',
                              icon: Compass,
                              desc: 'Will this branch limit my future jobs?',
                            },
                            {
                              id: 'Entrance exam choice',
                              icon: FileEdit,
                              desc: 'Which backup exams are worth taking?',
                            },
                            {
                              id: 'Location vs Profile',
                              icon: MapPin,
                              desc: 'Is moving far away worth the college tag?',
                            },
                            {
                              id: 'Future career fit',
                              icon: Target,
                              desc: "I don't even know what I want to do yet.",
                            },
                          ].map((c, i) => (
                            <OptionTile
                              key={c.id}
                              selected={confusion === c.id}
                              onClick={() => setConfusion(c.id)}
                              icon={c.icon}
                              label={c.id}
                              description={c.desc}
                              colorIdx={i}
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="sm:col-span-2 bg-[#080C24] p-6 border-b border-white/10 sm:border-b-0 sm:border-r">
                <p className="text-[10px] font-black uppercase tracking-widest text-white/25 mb-5">
                  Your Profile
                </p>
            ...
            >
              <div className="flex items-center gap-3 mb-5">
                ...
                <span className="text-[10px] font-black text-white/25 uppercase tracking-widest whitespace-nowrap">
                  Matched Mentors — {homeState || 'Your State'}
                </span>
                <div className="h-px flex-1 bg-white/10" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  ...
                  <div
                    key={i}
                    className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.05] p-4 select-none"
                  >
                    ...
                    <div className="flex items-center gap-3 opacity-50">
                      <div className="h-12 w-12 rounded-full bg-white/10 overflow-hidden blur-[3px] shrink-0">
                        <img
                          src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${m.name}&backgroundColor=1e293b`}
                          alt="Mentor"
                    ...
                    <div className="mt-3 space-y-1 opacity-50">
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-white/60">
                        ...
                        <span className="truncate">{homeState || 'Target State'}</span>
                      </div>
                    </div>
                  </div>
                ))}
