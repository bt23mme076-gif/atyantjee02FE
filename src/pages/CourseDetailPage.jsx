...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      getUserMe().then(r => setUser(r.user)).catch(() => {});
    }
                ...
                <div key={mod.id} className="border border-slate-200 rounded-xl overflow-hidden bg-white">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-slate-800">Module {idx + 1}: {mod.title}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      {!isUnlocked && <Lock className="w-4 h-4 text-slate-400" />}
                      {isUnlocked && !hasPurchased && mod.isFreePreview && <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded">Preview</span>}
                    </div>
                  </button>
                    ...
                    <div className="border-t border-slate-100 bg-white">
                      {mod.items.map(item => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            {item.type === 'video' ? (
                              <PlayCircle className={`w-5 h-5 ${isUnlocked ? 'text-[#FF6B2B]' : 'text-slate-400'}`} />
                            ) : (
                              <FileText className={`w-5 h-5 ${isUnlocked ? 'text-blue-500' : 'text-slate-400'}`} />
                            )}
                            <span className={`text-sm font-medium ${isUnlocked ? 'text-slate-700 group-hover:text-blue-700' : 'text-slate-500'}`}>
                              {item.title}
                            </span>
                          </div>
                          
                          <div className="flex items-center gap-3">
                            {item.durationLabel && <span className="text-xs font-medium text-slate-400">{item.durationLabel}</span>}
                            {isUnlocked ? (
                      ...
                      {mod.items.length === 0 && <div className="p-4 pl-12 text-sm text-slate-400">No items available yet.</div>}
                    </div>
                  )}
                </div>
              );
