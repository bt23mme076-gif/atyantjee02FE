...
function Section({ title, icon, children, className = '' }) {
  return (
    <div
      className={`bg-[#0F143C]/50 rounded-lg border border-white/10 shadow-sm overflow-hidden ${className}`}
    >
...
const inputCls =
  'w-full px-4 py-3 rounded-lg border border-white/10 bg-white/5 focus:outline-none focus:ring-2 focus:ring-[#FF6B2B]/40 transition text-sm text-white placeholder-slate-500';
  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.role === 'mentor') {
        ...
        const mappedBundles = (user.bundles || [])
          .map((b) => {
            if (
              [
                'Quick Clarity',
            ...
            if (
              [
                'Dream Seat Protection™',
                'dream-seat',
                'Complete Round Support',
                'complete-round',
              ].includes(b)
            )
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
              ...
              >
                <div className="relative w-24 h-24 sm:w-28 sm:h-28">
                  ...
                  {/* Overlay on hover */}
                  <div
                    className={`absolute inset-0 rounded-full bg-black/60 flex flex-col items-center justify-center transition-all duration-200 ${photoHover ? 'opacity-100' : 'opacity-0'}`}
                  >
                    {photoUploading ? (
                ...
                {user?.googleAvatar && !profilePhotoFilename ? (
                  <div
                    className="absolute bottom-0 right-0 w-5 h-5 bg-white rounded-full border-2 border-[#141936] flex items-center justify-center shadow-md"
                    title="Using Google profile photo"
                  >
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                <div className="mt-3 max-w-xs mx-auto sm:mx-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span
                      className={`text-[11px] font-black ${completion === 100 ? 'text-green-400' : 'text-[#FF6B2B]'}`}
                    >
                      {completion}%
                    </span>
                  </div>
                    ...
                    <div
                      ...
                      style={{
                        ...
                        background:
                          completion === 100
                            ? '#22c55e'
                            : 'linear-gradient(90deg, #FF6B2B, #FFB38E)',
                      }}
                    />
                  </div>
                </div>
        ...
        <div className="flex gap-2 p-1 bg-white/5 rounded-lg border border-white/10 overflow-x-auto scrollbar-none">
          {tabs.map((tab) => (
            ...
            >
              ...
              {tab.count > 0 && (
                <span
                  className={`text-[10px] font-black px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? 'bg-[#FF6B2B] text-white' : 'bg-white/10 text-white/60'}`}
                >
                  {tab.count}
                </span>
              )}
            ...
            {user.role === 'mentor' && (
              <>
                  ...
                  <div className="space-y-5">
                      ...
                      <Field label="College Name" required>
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (e.target.value !== 'OTHER_MANUAL') setCustomCollege('');
                          }}
                          className={`${selectCls} disabled:opacity-60`}
                        >
                          <option value="">
                            {collegeType ? 'Select College' : 'Choose Type First'}
                          </option>
                          {activeCollegeList.map((n) => (
                    ...
                    {college === 'OTHER_MANUAL' && (
                      <Field
                        ...
                        required
                        hint="Enter the full official name of your institution."
                      >
                        <input
                          type="text"
                          required
                        ...
                        <Field label="Department / Branch" required>
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.value !== 'OTHER_MANUAL') setCustomBranch('');
                            }}
                            className={selectCls}
                          >
                            <option value="" disabled>
                    ...
                    {branch === 'OTHER_MANUAL' && (
                      <Field
                        label="Type Your Department"
                        required
                        hint="Enter your department or specialized branch."
                      >
                        <input
                          type="text"
                          required
                  ...
                  >
                    <div className="relative">
                      ...
                      <div className="absolute bottom-3 right-4 text-xs font-medium text-slate-300 pointer-events-none">
                        <span
                          className={
                            bio.length >= MAX_BIO_CHARS ? 'text-red-500 font-semibold' : ''
                          }
                        >
                          {bio.length}
                        </span>
                ...
                <Section
                  ...
                  icon={<ShieldCheck className="w-4 h-4 text-[#FF6B2B]" />}
                >
                  <p className="text-xs text-slate-400 mb-4">
                    Managed by the Atyant team. To add or change packages, contact admin support.
                  </p>
          ...
          {loadingBookings ? (
            ...
            <div className="space-y-4">
              {bookings.map((booking) => (
                <div
                  key={booking._id}
                  className="bg-[#0F143C]/50 rounded-lg border border-white/10 shadow-sm p-5 hover:border-white/20 hover:shadow-md transition-all duration-200"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-sm font-black text-white">{booking.planTitle}</span>
                        <span className="text-[10px] font-bold bg-green-500/15 text-green-400 px-2.5 py-0.5 rounded-full border border-green-500/20">
                          Paid
                        </span>
                      </div>
                  ...
                  {booking.mentorId ? (
                    <div className="flex items-center gap-3 bg-white/5 p-3 rounded-lg border border-white/10 mb-4">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-[#FF6B2B] font-semibold">
                          {booking.mentorId.college}
                        </div>
                        <div className="text-[9px] text-slate-400">
                          AIR {booking.mentorId.rank || 'N/A'}
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="text-xs text-slate-400 italic mb-4">
                      No specific mentor assigned yet. Support will assign one!
                    </div>
                  )}
            ...
            <div className="bg-[#0F143C]/50 rounded-lg p-10 text-center border border-white/10 shadow-sm">
              ...
              <p className="text-sm font-bold text-white mb-1">
                No mentorship sessions purchased yet.
              </p>
              <p className="text-xs text-slate-400 mb-5 max-w-sm mx-auto">
                Get rank-based guidance from NIT/IIT seniors and secure your dream seat.
              </p>
              <button
            ...
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Upload your College ID or Aadhaar Card to get verified and appear to students.
                </p>
              </div>
              <div
            ...
            {idDocFilename && (
              <div className="mt-4 p-3 bg-white/5 rounded-lg border border-white/10 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  ...
                  <span
                    className="text-xs font-semibold text-white truncate max-w-[180px]"
                    title={idDocFilename}
                  >
                    {idDocFilename}
                  </span>
